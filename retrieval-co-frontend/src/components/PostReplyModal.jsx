...
export default function PostReplyModal({
    ...
    const isAuthor =
        ...
        post.author?.collegeId === currentUser?.code;

    const getTypeIcon = () => {
        switch (post.type) {
            case "lost":
                return <PackageSearch size={15} className="text-danger" />;
            case "found":
                return <PackageCheck size={15} className="text-success" />;
            case "borrow":
                return <Repeat size={15} className="text-primary-dim" />;
            default:
    ...
    const getTypeBadgeClass = () => {
        switch (post.type) {
            case "lost":
                return "px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-danger/10 text-danger border border-danger/15";
            case "found":
                return "px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-success/10 text-success border border-success/15";
            case "borrow":
    ...
    const handleSendReply = async () => {
        ...
        try {
            ...
            const res = await fetch(
                `${API_BASE}/api/posts/${post._id}/replies`,
                {
                    method: "POST",
                    headers: {
        ...
        return (
            ...
            >
                ...
                <div className="text-left">
                    <p className="text-[12px] font-medium text-text line-clamp-1 max-w-[140px]">
                        {post.title}
                    </p>
            ...
            >
                ...
                <div className="flex items-center justify-between p-4 border-b border-border shrink-0 bg-surface">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-[15px] font-display font-extrabold text-text line-clamp-1">
                                {post.title}
                            </h2>
                    ...
                    <div className="col-span-12 md:col-span-8 flex flex-col min-h-0 bg-surface">
                        ...
                        <div className="flex-1 overflow-y-auto p-5 space-y-4">
                            <h3 className="text-[10px] font-bold text-text-muted uppercase tracking-wider flex items-center gap-1.5 border-b border-border pb-2">
                                <MessageSquare size={12} /> Discussion Thread (
                                {localReplies.length} Messages)
                            </h3>
                                    ...
                                    {localReplies.map((reply, idx) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                        ...
                                                        style={{
                                                            background:
                                                                isPostAuthor
                                                                    ...
                                                                    : "linear-gradient(135deg, var(--border), var(--text-muted))",
                                                        }}
                                                        title="My Profile"
                                                    >
                                                        {initials}
                                                ...
                                                {/* Chat Bubble Box */}
                                                <div
                                                    className={`max-w-[75%] flex flex-col gap-1 ${isCurrentUser ? "items-end" : "items-start"}`}
                                                >
                                                    ...
                                                    {/* Actual bubble */}
                                                    <div
                                                        className={`p-3 rounded-xl border text-[13px] leading-relaxed ${
                                                            isCurrentUser
                                                    ...
                                                    <div
                                                        className={`text-[9.5px] text-text-muted mt-0.5 px-1 ${isCurrentUser ? "text-right" : "text-left"}`}
                                                    >
                                                        {new Date(
                                                            reply.createdAt,
                                                        ).toLocaleTimeString(
