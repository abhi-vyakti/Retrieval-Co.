...
export default function NavBar() {
    ...
    return (
        <div
            className={`fixed top-4 left-1/2 -translate-x-1/2 z-50 w-[calc(100%-32px)] px-2 transition-all duration-500 ease-in-out ${isAuthenticated ? "max-w-7xl" : "max-w-4xl"}`}
        >
            <nav className="relative flex items-center justify-between px-6 py-3.5 bg-card/85 backdrop-blur-[12px] border border-border shadow-card rounded-full transition-all duration-200">
                ...
                <Link
                    to="/"
                    className="flex items-center no-underline focus-visible:outline-none"
                    aria-label="Retrieval Co. Homepage"
                >
                ...
                {/* Nav Links (Desktop) */}
                <div
                    className={`hidden lg:flex items-center transition-all duration-300 ${isAuthenticated ? "flex-1 justify-center gap-16 lg:gap-24" : "gap-6"}`}
                >
                    {links.map((link) => {
                        const Icon = link.icon;
                        const isActive =
                            !link.onClick && location.pathname === link.path;
                ...
                <div className="absolute top-16 left-0 right-0 p-4 mx-2 bg-card/95 backdrop-blur-[16px] border border-border shadow-modal rounded-2xl flex flex-col gap-3 lg:hidden animate-in fade-in slide-in-from-top-3 duration-200">
                    ...
                    {isAuthenticated ? (
                        <div className="flex flex-col gap-3 pt-3 mt-1 border-t border-border">
                            ...
                            >
                                <span
                                    className="w-5 h-5 rounded-full flex items-center justify-center font-display font-bold text-[0.65rem] text-white"
                                    style={{
                                        background:
                                            "linear-gradient(135deg, var(--primary), var(--primary-dim))",
                                    }}
