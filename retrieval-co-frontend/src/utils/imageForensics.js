...
export async function analyzeImageAuthenticity(imageUrl, onProgress) {
        ...
        try {
            ...
            if (hasExposure) {
                ...
                if (tags.exif.ExposureTime)
                    parts.push(`Exp ${tags.exif.ExposureTime.description}s`);
                if (tags.exif.FNumber)
                    parts.push(`f/${tags.exif.FNumber.description}`);
                if (tags.exif.ISOSpeedRatings)
                    parts.push(`ISO ${tags.exif.ISOSpeedRatings.description}`);
                signals.push({
                    type: "positive",
                    label: "Exposure Data",
            ...
            if (tags.gps?.Latitude || tags.gps?.Longitude) {
                ...
                signals.push({
                    type: "positive",
                    label: "GPS Location",
                    detail: "Embedded geolocation data",
                });
            }
            ...
            const matchedAI = AI_SIGNATURES.find((sig) =>
    ...
    try {
        ...
        if (varRatio < 3) {
            ...
            signals.push({
                ...
                detail: "RGB variance is naturally distributed",
            });
        } else if (varRatio > 8) {
            ...
            signals.push({
                ...
                detail: "Atypical RGB variance ratio detected",
            });
        }
        ...
        signals.push({
            type: "neutral",
            label: "Pixel Analysis Skipped",
            detail: "Could not process pixels",
        });
    }
    ...
    if (isAIGenerated) {
        reason =
            "Multiple forensic signals indicate this image may be synthetically generated or heavily manipulated.";
    } else if (positiveCount >= 3) {
        reason =
            "Strong camera metadata and natural pixel patterns confirm authenticity.";
    } else if (positiveCount >= 1) {
        reason =
            "Some authenticity indicators present. Image appears legitimate.";
    } else {
        reason =
            "Limited metadata available, but pixel analysis does not suggest AI generation.";
    }
