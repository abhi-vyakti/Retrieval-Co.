...
import { MapPin, ShieldAlert, Award, Calendar, CheckCircle2, Shield } from 'lucide-react';
...
const MVP_HOTSPOTS = [
    { id: 1, name: "Sreenidhi Library", position: [17.4567, 78.6667], description: "High foot traffic area. Frequent lost IDs, wallets, and Books.", risk: "high" },
    { id: 2, name: "Campus Canteen", position: [17.4555, 78.6665], description: "Bustling area for lunch. Common place for lost bottles, umbrellas, and bags.", risk: "medium" },
    { id: 3, name: "Main Block", position: [17.4543, 78.6660], description: "Main classroom block. Electronics, laptops, and chargers usually found left behind here.", risk: "medium" }
];

const RECENT_RECOVERIES = [
    { id: 1, item: "Student ID Card", location: "Sreenidhi Library", time: "2 hrs ago" },
    { id: 2, item: "Casio Calculator", location: "Main Block Lab", time: "5 hrs ago" },
    { id: 3, item: "Puma Backpack", location: "Campus Canteen", time: "1 day ago" },
];
        ...
        <div className="min-h-screen bg-background pt-28 pb-16 px-4 md:px-8 max-w-7xl mx-auto flex flex-col gap-6">
            <div className="mb-4 border-b border-border pb-4">
                <h1 className="text-3xl font-display font-[800] text-text mb-2 tracking-tight">
                    Campus <span className="text-primary">Hotspots & Analytics</span>
                </h1>
                <p className="text-text-muted max-w-2xl text-[14px]">
                    Real-time campus risk heatmap. View where items go missing most on campus and track recent successful returns.
                </p>
            </div>
                    ...
                    <div className="glass-panel rounded-xl overflow-hidden shadow-sm h-[400px] lg:h-[520px]">
                        <MapContainer center={mapCenter} zoom={16} style={{ height: '100%', width: '100%', zIndex: 1 }}>
                            ...
                            {MVP_HOTSPOTS.map((hotspot) => (
                                <Marker key={hotspot.id} position={hotspot.position} icon={hotspot.risk === 'high' ? redIcon : orangeIcon}>
                                        ...
                                        <div className="p-1 min-w-[200px]">
                                            ...
                                            <div className="flex items-center justify-between">
                                                <div className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded border ${
                                                    ...
                                                    {hotspot.risk === 'high' ? 'High Risk' : 'Medium Risk'} Hotspot
                                                </div>
                                            </div>
                                        </div>
                                    </Popup>
                ...
                <div className="space-y-6">
                    ...
                    <div className="glass-panel p-5 rounded-xl flex flex-col gap-4">
                        <h3 className="text-xs font-bold text-text uppercase tracking-wider flex items-center gap-1.5">
                            <CheckCircle2 size={14} className="text-success" /> Recent Recoveries
                        </h3>
                        <div className="flex flex-col gap-3">
                            {RECENT_RECOVERIES.map(rec => (
                                <div key={rec.id} className="flex flex-col p-3 rounded-lg bg-surface border border-border gap-1 animate-in fade-in duration-200">
                                    <div className="flex justify-between items-center text-xs">
                                        <span className="font-bold text-text">{rec.item}</span>
                                        <span className="text-[10px] text-text-muted">{rec.time}</span>
                                    </div>
                                    <div className="text-[11px] text-text-muted flex items-center gap-1">
                                        <MapPin size={10} /> Returned near {rec.location}
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="glass-panel p-5 rounded-xl flex flex-col gap-4">
                        <h3 className="text-xs font-bold text-text uppercase tracking-wider flex items-center gap-1.5">
                            <ShieldAlert size={14} className="text-warning" /> Common Loss Spots
                        </h3>
                        <div className="flex flex-col gap-2">
                            {COMMON_LOCATIONS.map((loc, idx) => (
                                <div key={idx} className="flex justify-between items-center p-3 rounded-lg bg-surface border border-border text-xs">
                                    <div className="flex flex-col gap-0.5">
                                        <span className="font-semibold text-text">{loc.name}</span>
                                        <span className={`text-[10px] font-medium ${loc.status === 'High Risk' ? 'text-danger' : 'text-warning'}`}>{loc.status}</span>
                                    </div>
                    ...
                    <div className="glass-panel p-5 rounded-xl flex items-center justify-between shadow-sm">
                        <div>
                            <span className="text-[11px] font-bold text-text-muted uppercase tracking-wider block mb-1">Campus Safety Rating</span>
                            <span className="text-2xl font-display font-extrabold text-success">9.2 / 10</span>
                        </div>
