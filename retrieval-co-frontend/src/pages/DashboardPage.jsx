...
import { 
    Search, Filter, AlertCircle, PackageSearch, Plus, 
    UserSquare, Bell, TrendingUp, CheckCircle, MessageSquare, 
    MapPin, Flame, Award, Calendar, Grid, Laptop, Pencil,
    CreditCard, BookOpen, Shirt, Beaker, MoreHorizontal 
} from 'lucide-react';
...
export default function DashboardPage() {
                ...
                setPosts(prevPosts => prevPosts.map(p => {
                    if (p._id === postId) {
                        return {
                            ...
                            isUrgent: isUrgent !== undefined ? isUrgent : p.isUrgent
                        };
                    }
                    return p;
                }));
        ...
        const handleOpenPostReply = async (e) => {
            ...
            if (postToReply) {
                ...
                if (postToReply.type === 'borrow' && activeTab !== 'borrow') {
                    setActiveTab('borrow');
                } else if (postToReply.type !== 'borrow' && activeTab !== 'lost_found') {
    ...
    const notifications = [];
    posts.forEach(post => {
        if (post.author?._id === user?.id || post.author?.collegeId === user?.code) {
    ...
    const filteredPosts = posts.filter(post => {
        if (activeChipFilter === 'today') {
            const postDate = new Date(post.createdAt || post.datetime).toDateString();
        ...
        if (activeChipFilter === 'nearby') {
            ...
            const isNearby = loc.includes('canteen') || loc.includes('library') || loc.includes('block') || loc.includes('lab') || loc.includes('hall');
            if (!isNearby) return false;
        }
        if (activeChipFilter === 'my_posts') {
            if (post.author?._id !== user?.id && post.author?.collegeId !== user?.code) return false;
        }
        if (activeChipFilter === 'borrow' && post.type !== 'borrow') return false;
        ...
        <div className="min-h-screen bg-background pt-28 pb-20 px-4 md:px-8 max-w-7xl mx-auto flex flex-col gap-8">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 py-4 border-b border-border">
                <div>
                    ...
                    <p className="text-text-muted text-[14px]">Explore lost, found, and borrowable items on campus.</p>
                </div>

                <div className="flex items-center gap-3 w-full md:w-auto">
                    <Link to="/my-posts" className="btn-ghost flex items-center gap-2 no-underline text-xs">
                        <UserSquare size={15} />
                        <span>My Posts</span>
                    </Link>
                </div>
            ...
            <div className="flex items-center justify-between border-b border-border pb-4">
                <div className="relative flex p-1 bg-surface border border-border rounded-full w-[260px] overflow-hidden">
                    ...
                    <div 
                        ...
                        style={{
                            width: 'calc(50% - 4px)',
                            transform: activeTab === 'lost_found' ? 'translateX(0)' : 'translateX(100%)'
                        }}
                    />
                ...
                <Link to="/hotspots" className="btn-ghost !rounded-full text-xs">
                    View Hotspots
                </Link>
            </div>
                ...
                <div className="lg:col-span-1 space-y-6 lg:sticky lg:top-28">
                    ...
                    <div className="glass-panel p-5 rounded-xl flex flex-col gap-4">
                        <h3 className="text-xs font-bold text-text uppercase tracking-wider flex items-center gap-1.5 mb-1">
                            <PackageSearch size={14} className="text-primary" /> Categories
                        </h3>
                        <div className="flex flex-col gap-1.5">
                            {[
                                { name: 'All', label: 'All Categories', icon: Grid },
                                { name: 'Electronics', label: 'Electronics', icon: Laptop },
                                { name: 'Stationery', label: 'Stationery', icon: Pencil },
                                { name: 'ID Cards', label: 'ID Cards', icon: CreditCard },
                                { name: 'Books', label: 'Books', icon: BookOpen },
                                { name: 'Clothing', label: 'Clothing', icon: Shirt },
                                { name: 'Lab Equipment', label: 'Lab Equipment', icon: Beaker },
                                ...
                                return (
                                    ...
                                    >
                                        <Icon size={16} className={isSelected ? 'text-white' : 'text-text-muted/70'} />
                ...
                <div className="lg:col-span-2 space-y-6">
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-24 bg-card rounded-2xl border border-border border-dashed text-center p-8">
                            <PackageSearch className="text-text-muted mb-5" size={32} />
                            <h3 className="text-xl font-display font-bold text-text mb-2">No items reported</h3>
                            <p className="text-text-muted text-[13px] max-w-sm mb-6 leading-relaxed">
                                Start by posting a lost item, a found object, or a borrow request.
                            </p>
                            <Link to="/create" className="btn-primary no-underline text-xs">
                                Create Post
                            </Link>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 animate-in fade-in duration-300">
                             {filteredPosts.map(post => (
                                <PostCard
                                    ...
                                    post={post}
                                    onReply={handleReply}
                                    isAuthor={post.author?._id === user?.id || post.author?.collegeId === user?.code}
            ...
            <QRReturnModal
                ...
                post={selectedQRPost}
                isOwner={selectedQRPost ? (selectedQRPost.author?._id === user?.id || selectedQRPost.author?.collegeId === user?.code) : false}
                onSuccessCallback={() => {
                    setIsQRModalOpen(false);
                    fetchPosts();
                }}
