...
export default function LeaderboardPage() {
    ...
    const getRankBadge = (rank) => {
        switch (rank) {
            case 1:
                return (
                    <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-yellow-500/10 text-yellow-500 border border-yellow-500/25 font-bold text-xs">
                        1
                    </span>
                );
            case 2:
                return (
                    <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-zinc-400/10 text-zinc-400 border border-zinc-400/25 font-bold text-xs">
                        2
                    </span>
                );
            case 3:
                return (
                    <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-orange-500/10 text-orange-500 border border-orange-500/25 font-bold text-xs">
                        3
                    </span>
                );
            default:
                return (
                    <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-surface text-text-muted border border-border font-bold text-xs">
                        {rank}
                    </span>
                );
        }
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-2">
                {leaders.slice(0, 3).map((user, index) => {
                    ...
                    const returnCount = Math.max(
                        1,
                        Math.round(user.karmaPoints / 40),
                    );
                    return (
                        <div
                            key={user.id}
                            className="glass-panel p-5 rounded-xl flex items-center gap-4 relative overflow-hidden"
                        >
                            ...
                            <div
                                className="w-10 h-10 rounded-full flex items-center justify-center text-xs font-display font-extrabold text-white"
                                style={{
                                    background:
                            ...
                            <div className="flex-grow min-w-0">
                                ...
                                <div className="text-[10px] text-text-muted mt-0.5 flex items-center gap-1.5">
                                    <span className="bg-surface px-1.5 py-0.5 rounded text-text border border-border font-medium text-[9px]">
                        ...
                        <tbody>
                            {loading ? (
                                Array.from({ length: 5 }).map((_, i) => (
                                    <tr
                                        key={i}
                                        className="border-b border-border/40 animate-pulse"
                                    >
                                ...
                                <tr>
                                    ...
                                    >
                                        <Trophy
                                            size={40}
                                            className="text-text-muted mb-3 mx-auto"
                                        />
                                        <h3 className="text-base font-display font-bold text-text mb-1">
                                            No contributors yet
                                        </h3>
                                        <p className="text-xs text-text-muted">
                                            Return an item on campus to claim
                                            the top spot!
                                        </p>
                                    </td>
                                </tr>
            ...
            <div className="md:hidden flex flex-col gap-3">
                {loading ? (
                    Array.from({ length: 5 }).map((_, i) => (
                        <div
                            key={i}
                            className="glass-panel p-4 rounded-xl flex items-center gap-3 animate-pulse"
                        >
                    ...
                    <div className="glass-panel p-8 rounded-xl text-center">
                        ...
                        <p className="text-xs text-text-muted">
                            Return an item on campus to claim the top spot!
                        </p>
                    </div>
                ) : (
                    leaders.map((user, index) => {
                        ...
                        return (
                            <div
                                key={user.id}
                                className="glass-panel p-4 rounded-xl flex flex-col gap-3 relative"
                            >
