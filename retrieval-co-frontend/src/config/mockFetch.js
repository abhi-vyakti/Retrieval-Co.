...
const DEFAULT_POSTS = [
    {
        ...
        status: 'open',
        isUrgent: true,
        author: { _id: 'user_ananyasingh', name: 'Ananya Singh', collegeId: '24CIV8765', karma: 134 },
        replies: [],
    ...
    },
    {
        ...
        author: { _id: 'user_rahulverma', name: 'Rahul Verma', collegeId: '21MEC5678', karma: 189 },
        replies: [],
    ...
    },
    {
        ...
        status: 'open',
        isUrgent: true,
        author: { _id: 'user_priyanair', name: 'Priya Nair', collegeId: '23ECE4321', karma: 247 },
        replies: [],
    ...
    },
    {
        ...
        category: 'Others',
        description: 'Left a blue Milton stainless steel water bottle in Seminar Hall 2.',
        location: 'Seminar Hall 2',
        datetime: new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString(),
        status: 'open',
        isUrgent: false,
        author: { _id: 'user_kiransharma', name: 'Kiran Sharma', collegeId: '22BCE1234', karma: 312 },
        replies: [],
...
window.fetch = async function (url, options = {}) {
    ...
    if (isDemoMode && (urlString.includes('/api/') || urlString.startsWith('/api/'))) {
        ...
        try {
            ...
            if (path === '/api/auth/login') {
                ...
                let user = users.find(u => u.collegeId === code || u._id === code);
                if (!user) {
                    user = {
            ...
            if (path === '/api/posts' && method === 'POST') {
                ...
                const newPost = {
                    ...
                    author: {
                        ...
                        name: body.isAnonymous ? 'Anonymous Student' : activeUser.name,
                        collegeId: body.isAnonymous ? 'Hidden' : activeUser.collegeId,
            ...
            if (statusMatch && (method === 'PUT' || method === 'PATCH')) {
                ...
                if (typeof body.isUrgent !== 'undefined') post.isUrgent = body.isUrgent;
            ...
            if (path === '/api/karma/leaderboard' && method === 'GET') {
                ...
                const leaderboard = [...users].sort((a, b) => (b.karma || 0) - (a.karma || 0));
                return new Response(JSON.stringify({
            ...
            // ─── HANDOFF / QR ───
            const qrGenMatch = path.match(/^\/api\/return\/([^\/]+)\/generate-qr$/);
            ...
            if (path === '/api/return/confirm-qr' && method === 'POST') {
                ...
                if (activeUserId !== ownerId) {
                    const users = getUsers();
                    const userIndex = users.findIndex(u => u._id === activeUserId);
                    if (userIndex >= 0) {
                        users[userIndex].karma = (users[userIndex].karma || 0) + 50;
                        saveUsers(users);
            ...
            if (path === '/api/ai/parse-post' && method === 'POST') {
                ...
                let type = 'lost';
                if (textLower.includes('found') || textLower.includes('discovered')) type = 'found';
                else if (textLower.includes('borrow') || textLower.includes('lend') || textLower.includes('need')) type = 'borrow';

                let category = 'Others';
                if (textLower.includes('phone') || textLower.includes('laptop') || textLower.includes('charger') || textLower.includes('calculator') || textLower.includes('casio')) {
                    category = 'Electronics';
                } else if (textLower.includes('pen') || textLower.includes('pencil') || textLower.includes('drafter') || textLower.includes('ruler')) {
                    category = 'Stationery';
                } else if (textLower.includes('id') || textLower.includes('card') || textLower.includes('lanyard')) {
                    category = 'ID Cards';
                } else if (textLower.includes('book') || textLower.includes('textbook') || textLower.includes('notebook')) {
                    category = 'Books';
                } else if (textLower.includes('jacket') || textLower.includes('bottle') || textLower.includes('umbrella')) {
                    category = 'Clothing';
                }
                ...
                return new Response(JSON.stringify({
                    ...
                    parsed: {
                        ...
                        isUrgent: textLower.includes('urgent') || textLower.includes('fast') || textLower.includes('immediately')
                    }
                }), { status: 200, headers: { 'Content-Type': 'application/json' } });
            }
            ...
            if (path === '/api/ai/find-matches' && method === 'POST') {
                ...
                const candidates = posts.filter(p => p.type === oppositeType && p.category === category && p.status === 'open');
            ...
            return new Response(JSON.stringify({ error: 'Mock Server Error: ' + error.message }), { status: 500 });
        }
    }
